...
use crate::pdf_lib::appearanceprintermark::appearanceprintermark_type;
...
use std::rc::Rc;
pub fn annotprintermark_type<'a>(tctx: &'a mut TypeCheckContext) -> Rc<TypeCheck> {
    let assignment_22 = exdataprojection_type(tctx);
    let assignment_21 = exdatamarkupgeo_type(tctx);
    let assignment_20 = exdata3dmarkup_type(tctx);
    ...
    let assignment_7 = appearanceprintermark_type(tctx);
    ...
    let choices_mn = ChoicePred(
        String::from("Invalid MN"),
        vec![
            PDFObjT::Name(NameT::new(Vec::from("ColorBar"))),
            PDFObjT::Name(NameT::new(Vec::from("RegistrationTarget"))),
        ],
    );
    let choices_subtype = ChoicePred(
        String::from("Invalid Subtype"),
        vec![PDFObjT::Name(NameT::new(Vec::from("PrinterMark")))],
    );
