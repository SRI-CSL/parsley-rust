...
use crate::pdf_lib::addactionscreenannotation::addactionscreenannotation_type;
...
use std::rc::Rc;
pub fn annotscreen_type<'a>(tctx: &'a mut TypeCheckContext) -> Rc<TypeCheck> {
    let assignment_45 = exdataprojection_type(tctx);
    let assignment_44 = exdatamarkupgeo_type(tctx);
    let assignment_43 = exdata3dmarkup_type(tctx);
    let assignment_42 = addactionscreenannotation_type(tctx);
    ...
    let assignment_21 = appearancecharacteristics_type(tctx);
    ...
    let dis_10 = TypeCheck::new(tctx, "aa", Rc::new(PDFType::Disjunct(vec![assignment_42])));
    ...
    let dis_8 = TypeCheck::new(tctx, "mk", Rc::new(PDFType::Disjunct(vec![assignment_21])));
    ...
    let choices_subtype = ChoicePred(
        ...
        vec![PDFObjT::Name(NameT::new(Vec::from("Screen")))],
    );
