...
use std::rc::Rc;
pub fn minimumscreensize_type<'a>(tctx: &'a mut TypeCheckContext) -> Rc<TypeCheck> {
    let assignment_0 = arrayof_2integers_type(tctx);
    let dis_0 = TypeCheck::new(tctx, "v", Rc::new(PDFType::Disjunct(vec![assignment_0])));
    ...
    let choices_type = ChoicePred(
        ...
        vec![PDFObjT::Name(NameT::new(Vec::from("MinScreenSize")))],
    );
