...
use std::rc::Rc;
pub fn annotpopup_type<'a>(tctx: &'a mut TypeCheckContext) -> Rc<TypeCheck> {
    let assignment_49 = exdataprojection_type(tctx);
    let assignment_48 = exdatamarkupgeo_type(tctx);
    let assignment_47 = exdata3dmarkup_type(tctx);
    let assignment_bool_46 =
        TypeCheck::new(tctx, "open", Rc::new(PDFType::PrimType(PDFPrimType::Bool)));
    let assignment_45 = annotrichmedia_type(tctx);
    let assignment_44 = annotprojection_type(tctx);
    let assignment_43 = annotredact_type(tctx);
    let assignment_42 = annot3d_type(tctx);
    let assignment_41 = annotwatermark_type(tctx);
    let assignment_40 = annotprintermark_type(tctx);
    let assignment_39 = annotwidget_type(tctx);
    let assignment_38 = annotscreen_type(tctx);
    let assignment_37 = annotmovie_type(tctx);
    let assignment_36 = annotsound_type(tctx);
    let assignment_35 = annotfileattachment_type(tctx);
    let assignment_34 = annotpopup_type(tctx);
    let assignment_33 = annotink_type(tctx);
    let assignment_32 = annotstamp_type(tctx);
    let assignment_31 = annotcaret_type(tctx);
    let assignment_30 = annotstrikeout_type(tctx);
    let assignment_29 = annotsquiggly_type(tctx);
    let assignment_28 = annotunderline_type(tctx);
    let assignment_27 = annothighlight_type(tctx);
    let assignment_26 = annotpolygon_type(tctx);
    let assignment_25 = annotcircle_type(tctx);
    let assignment_24 = annotsquare_type(tctx);
    let assignment_23 = annotline_type(tctx);
    let assignment_22 = annotfreetext_type(tctx);
    let assignment_21 = annotlink_type(tctx);
    let assignment_20 = annottext_type(tctx);
    ...
    let choices_subtype = ChoicePred(
        ...
        vec![PDFObjT::Name(NameT::new(Vec::from("Popup")))],
    );
    ...
    let open_field = DictEntry {
        key: Vec::from("Open"),
        chk: assignment_bool_46,
