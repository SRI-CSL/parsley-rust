...
use super::pdf_prim::{WhitespaceEOL, WhitespaceNoEOL, Comment};
use super::pdf_prim::{Boolean, Null, Number, HexString, RawLiteralString};
...
impl IndirectP {
    pub fn parse(self, buf: &mut ParseBuffer) -> Result<IndirectT, ErrorKind> {
        ...
        Ok(IndirectT::new(num.int_val(), gen.int_val(), Box::new(o)))
    }
}
...
pub struct PDFObjP {
    // The parser is implemented as a set of mutually recursive
    // functions, one for each variant.
}
impl PDFObjP {
    pub fn new() -> PDFObjP {
        PDFObjP {}
    ...
    // The top-level object parser.  The tricky thing here is
    // dispatching numerical prefixes.
    pub fn parse(self, buf: &mut ParseBuffer) -> Result<PDFObjT, ErrorKind> {
        ...
        match c {
            ...
            Some(115) => { // 's'
                ...
                Ok(PDFObjT::StreamContent(s.parse(buf)?))
            },
            // TODO: numbers
            Some(_)   => Err(ErrorKind::EndOfBuffer)
        }
    }
}
