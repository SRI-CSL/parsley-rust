...
use std::rc::Rc;
pub fn annottext_type<'a>(tctx: &'a mut TypeCheckContext) -> Rc<TypeCheck> {
    ...
    let assignment_54 = TypeCheck::new(
        tctx,
        "statemodel",
        Rc::new(PDFType::PrimType(PDFPrimType::String)),
    );
    let assignment_53 = TypeCheck::new(
        tctx,
        "state",
        Rc::new(PDFType::PrimType(PDFPrimType::String)),
    );
    let assignment_bool_52 =
    ...
    let choices_name = ChoicePred(
        String::from("Invalid Name"),
        vec![
            PDFObjT::Name(NameT::new(Vec::from("Comment"))),
            PDFObjT::Name(NameT::new(Vec::from("Key"))),
            PDFObjT::Name(NameT::new(Vec::from("Note"))),
            PDFObjT::Name(NameT::new(Vec::from("Help"))),
            PDFObjT::Name(NameT::new(Vec::from("NewParagraph"))),
            PDFObjT::Name(NameT::new(Vec::from("Paragraph"))),
            PDFObjT::Name(NameT::new(Vec::from("Insert"))),
        ],
    );
    ...
    let choices_subtype = ChoicePred(
        ...
        vec![PDFObjT::Name(NameT::new(Vec::from("Text")))],
    );
    ...
    let state_field = DictEntry {
        key: Vec::from("State"),
        chk: assignment_53,

        opt: DictKeySpec::Optional,
    };
    let statemodel_field = DictEntry {
        key: Vec::from("StateModel"),
        chk: assignment_54,
