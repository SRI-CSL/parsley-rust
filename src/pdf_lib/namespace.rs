...
use std::rc::Rc;
pub fn namespace_type<'a>(tctx: &'a mut TypeCheckContext) -> Rc<TypeCheck> {
    let assignment_2 = rolemap_type(tctx);
    let assignment_1 = filespecification_type(tctx);
    let assignment_0 = TypeCheck::new(tctx, "ns", Rc::new(PDFType::PrimType(PDFPrimType::String)));
    ...
    let choices_type = ChoicePred(
        ...
        vec![PDFObjT::Name(NameT::new(Vec::from("Namespace")))],
    );
    ...
    let rolemapns_field = DictEntry {
        key: Vec::from("RoleMapNS"),
        chk: dis_1,
        opt: DictKeySpec::Optional,
    };
    TypeCheck::new(
