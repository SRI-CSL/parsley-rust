...
use crate::pdf_lib::target::target_type;
use std::rc::Rc;
pub fn actiongotoe_type<'a>(tctx: &'a mut TypeCheckContext) -> Rc<TypeCheck> {
    let assignment_27 = target_type(tctx);
    let assignment_bool_26 = TypeCheck::new(
        tctx,
        "newwindow",
    ...
    let dis_3 = TypeCheck::new(tctx, "t", Rc::new(PDFType::Disjunct(vec![assignment_27])));
    ...
    let choices_s = ChoicePred(
        ...
        vec![PDFObjT::Name(NameT::new(Vec::from("GoToE")))],
    );
