...
use crate::pdf_lib::richmediacommand::richmediacommand_type;
use crate::pdf_lib::richmediainstances::richmediainstances_type;
use std::rc::Rc;
pub fn actionrichmediaexecute_type<'a>(tctx: &'a mut TypeCheckContext) -> Rc<TypeCheck> {
    let assignment_23 = richmediacommand_type(tctx);
    let assignment_22 = richmediainstances_type(tctx);
    let assignment_21 = annotrichmedia_type(tctx);
    ...
    let dis_3 = TypeCheck::new(tctx, "cmd", Rc::new(PDFType::Disjunct(vec![assignment_23])));
    let dis_2 = TypeCheck::new(tctx, "ti", Rc::new(PDFType::Disjunct(vec![assignment_22])));
    let dis_1 = TypeCheck::new(tctx, "ta", Rc::new(PDFType::Disjunct(vec![assignment_21])));
    ...
    let choices_s = ChoicePred(
        ...
        vec![PDFObjT::Name(NameT::new(Vec::from("RichMediaExecute")))],
    );
    ...
    TypeCheck::new(
        tctx,
        "actionrichmediaexecute",
        Rc::new(PDFType::Dict(vec![
            type_field, s_field, next_field, ta_field, ti_field, cmd_field,
        ])),
    )
}
