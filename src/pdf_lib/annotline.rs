...
use crate::pdf_lib::arrayof_4numbers::arrayof_4numbers_type;
...
use std::rc::Rc;
pub fn annotline_type<'a>(tctx: &'a mut TypeCheckContext) -> Rc<TypeCheck> {
    let assignment_64 = exdataprojection_type(tctx);
    let assignment_63 = exdatamarkupgeo_type(tctx);
    let assignment_62 = exdata3dmarkup_type(tctx);
    let assignment_61 = arrayof_2numbers_type(tctx);
    let assignment_60 = measuregeo_type(tctx);
    let assignment_59 = measurerl_type(tctx);
    let assignment_58 = TypeCheck::new(tctx, "llo", Rc::new(PDFType::PrimType(PDFPrimType::Real)));
    let assignment_bool_57 =
        TypeCheck::new(tctx, "cap", Rc::new(PDFType::PrimType(PDFPrimType::Bool)));
    let assignment_56 = TypeCheck::new(tctx, "lle", Rc::new(PDFType::PrimType(PDFPrimType::Real)));
    let assignment_55 = TypeCheck::new(tctx, "ll", Rc::new(PDFType::PrimType(PDFPrimType::Real)));
    let assignment_53 = arrayof_2lineendingsnames_type(tctx);
    let assignment_52 = borderstyle_type(tctx);
    let assignment_51 = arrayof_4numbers_type(tctx);
    ...
    let assignment_54 = arrayof_4numberscolorannotation_type(tctx);
    ...
    let dis_17 = TypeCheck::new(tctx, "co", Rc::new(PDFType::Disjunct(vec![assignment_61])));
    let dis_16 = TypeCheck::new(
        tctx,
        "measure",
        Rc::new(PDFType::Disjunct(vec![assignment_59, assignment_60])),
    );
    let dis_15 = TypeCheck::new(tctx, "ic", Rc::new(PDFType::Disjunct(vec![assignment_54])));
    let dis_14 = TypeCheck::new(tctx, "le", Rc::new(PDFType::Disjunct(vec![assignment_53])));
    let dis_13 = TypeCheck::new(tctx, "bs", Rc::new(PDFType::Disjunct(vec![assignment_52])));
    let dis_12 = TypeCheck::new(tctx, "l", Rc::new(PDFType::Disjunct(vec![assignment_51])));
    ...
    let choices_cp = ChoicePred(
        String::from("Invalid CP"),
        vec![
            PDFObjT::Name(NameT::new(Vec::from("Inline"))),
            PDFObjT::Name(NameT::new(Vec::from("Top"))),
        ],
    );
    let choices_it = ChoicePred(
        String::from("Invalid IT"),
        vec![
            PDFObjT::Name(NameT::new(Vec::from("LineArrow"))),
            PDFObjT::Name(NameT::new(Vec::from("LineDimension"))),
        ],
    );
    ...
    let choices_subtype = ChoicePred(
        ...
        vec![PDFObjT::Name(NameT::new(Vec::from("Line")))],
    );
    ...
    let ll_field = DictEntry {
        key: Vec::from("LL"),
        chk: assignment_55,

        opt: DictKeySpec::Optional,
    };
    let lle_field = DictEntry {
        key: Vec::from("LLE"),
        chk: assignment_56,

        opt: DictKeySpec::Optional,
    };
    let cap_field = DictEntry {
        key: Vec::from("Cap"),
        chk: assignment_bool_57,
