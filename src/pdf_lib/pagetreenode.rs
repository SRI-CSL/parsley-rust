...
use std::rc::Rc;
pub fn pagetreenode_type<'a>(tctx: &'a mut TypeCheckContext) -> Rc<TypeCheck> {
    ...
    let assignment_2 = arrayofpagetreenodekids_type(tctx);
    let assignment_1 = pagetreenoderoot_type(tctx);
    let assignment_0 = pagetreenode_type(tctx);
    let assignments_disjuncts_1 = Rc::new(PDFType::Disjunct(vec![assignment_rectangle_6]));
    let assignments_disjuncts_0 = Rc::new(PDFType::Disjunct(vec![assignment_rectangle_5]));
    let dis_4 = TypeCheck::new(tctx, "cropbox", assignments_disjuncts_1);
    let dis_3 = TypeCheck::new(tctx, "mediabox", assignments_disjuncts_0);
    let dis_2 = TypeCheck::new(
        tctx,
        "resources",
        Rc::new(PDFType::Disjunct(vec![assignment_4])),
    );
    let dis_1 = TypeCheck::new(tctx, "kids", Rc::new(PDFType::Disjunct(vec![assignment_2])));
