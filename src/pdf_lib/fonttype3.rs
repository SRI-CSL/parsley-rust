...
use crate::pdf_lib::stream::stream_type;
    ...
    let choices_subtype = ChoicesPred(
        ...
        vec![ 
         PDFObjT::Name(NameT::new(Vec::from("Type3"))),
           ],
     );
pub fn fonttype3_type(tctx: &mut TypeCheckContext) -> Rc<TypeCheck> {
    ...
    let fontmatrix_field = DictEntry { 
       key: Vec::from("FontMatrix"), 
    ...
    let charprocs_field = DictEntry { 
       key: Vec::from("CharProcs"), 
       chk: Rc::new(PDFType::Disjunct(vec![
         [_type,
         c_type,
         h_type,
