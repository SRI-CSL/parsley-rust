...
use std::rc::Rc;
pub fn actionresetform_type<'a>(tctx: &'a mut TypeCheckContext) -> Rc<TypeCheck> {
    let assignment_integer_22 = TypeCheck::new(
        tctx,
        "flags",
        Rc::new(PDFType::PrimType(PDFPrimType::Integer)),
    );
    let assignment_21 = arrayoffieldid_type(tctx);
    ...
    let choices_s = ChoicePred(
        ...
        vec![PDFObjT::Name(NameT::new(Vec::from("ResetForm")))],
    );
