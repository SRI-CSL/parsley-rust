...
use crate::pdf_lib::microsoftwindowslaunchparam::microsoftwindowslaunchparam_type;
...
use std::rc::Rc;
pub fn actionlaunch_type<'a>(tctx: &'a mut TypeCheckContext) -> Rc<TypeCheck> {
    let assignment_bool_25 = TypeCheck::new(
        tctx,
        "newwindow",
        Rc::new(PDFType::PrimType(PDFPrimType::Bool)),
    );
    let assignment_null_23 = Rc::new(PDFType::PrimType(PDFPrimType::Null));
    let assignment_null_24 = Rc::new(PDFType::PrimType(PDFPrimType::Null));
    let assignment_22 = microsoftwindowslaunchparam_type(tctx);
    let assignment_21 = filespecification_type(tctx);
    ...
    let dis_2 = TypeCheck::new(tctx, "win", Rc::new(PDFType::Disjunct(vec![assignment_22])));
    let dis_1 = TypeCheck::new(tctx, "f", Rc::new(PDFType::Disjunct(vec![assignment_21])));
    ...
    let choices_s = ChoicePred(
        ...
        vec![PDFObjT::Name(NameT::new(Vec::from("Launch")))],
    );
    ...
    let mac_field = DictEntry {
        key: Vec::from("Mac"),
        chk: TypeCheck::new(tctx, "mac", assignment_null_23),
        opt: DictKeySpec::Optional,
    };
    let unix_field = DictEntry {
        key: Vec::from("Unix"),
        chk: TypeCheck::new(tctx, "unix", assignment_null_24),
        opt: DictKeySpec::Optional,
    };
    let newwindow_field = DictEntry {
        key: Vec::from("NewWindow"),
        chk: assignment_bool_25,
