...
use std::rc::Rc;
pub fn annotfileattachment_type<'a>(tctx: &'a mut TypeCheckContext) -> Rc<TypeCheck> {
    let assignment_55 = exdataprojection_type(tctx);
    let assignment_54 = exdatamarkupgeo_type(tctx);
    let assignment_53 = exdata3dmarkup_type(tctx);
    ...
    let assignment_52 = filespecification_type(tctx);
    ...
    let dis_12 = TypeCheck::new(tctx, "fs", Rc::new(PDFType::Disjunct(vec![assignment_52])));
    ...
    let choices_name = ChoicePred(
        ...
        vec![
            PDFObjT::Name(NameT::new(Vec::from("Graph"))),
            PDFObjT::Name(NameT::new(Vec::from("PushPin"))),
            PDFObjT::Name(NameT::new(Vec::from("Paperclip"))),
            PDFObjT::Name(NameT::new(Vec::from("Tag"))),
        ],
    );
    ...
    let choices_subtype = ChoicePred(
        ...
        vec![PDFObjT::Name(NameT::new(Vec::from("FileAttachment")))],
    );
