...
use std::rc::Rc;
pub fn annotcircle_type<'a>(tctx: &'a mut TypeCheckContext) -> Rc<TypeCheck> {
    ...
    let assignment_54 = bordereffect_type(tctx);
    ...
    let assignment_rectangle_55 = mk_rectangle_typchk(tctx);
    let assignments_disjuncts_3 = Rc::new(PDFType::Disjunct(vec![assignment_rectangle_55]));
    ...
    let dis_15 = TypeCheck::new(tctx, "rd", assignments_disjuncts_3);
    let dis_14 = TypeCheck::new(tctx, "be", Rc::new(PDFType::Disjunct(vec![assignment_54])));
    let dis_13 = TypeCheck::new(tctx, "ic", Rc::new(PDFType::Disjunct(vec![assignment_53])));
    let dis_12 = TypeCheck::new(tctx, "bs", Rc::new(PDFType::Disjunct(vec![assignment_52])));
    ...
    let choices_subtype = ChoicePred(
        ...
        vec![PDFObjT::Name(NameT::new(Vec::from("Circle")))],
    );
