...
use std::rc::Rc;
pub fn actiongotodp_type<'a>(tctx: &'a mut TypeCheckContext) -> Rc<TypeCheck> {
    let assignment_21 = dpart_type(tctx);
    ...
    let dis_1 = TypeCheck::new(tctx, "dp", Rc::new(PDFType::Disjunct(vec![assignment_21])));
    ...
    let choices_s = ChoicePred(
        ...
        vec![PDFObjT::Name(NameT::new(Vec::from("GoToDp")))],
    );
    ...
    TypeCheck::new(
        ...
        Rc::new(PDFType::Dict(vec![
            type_field, s_field, next_field, dp_field,
        ])),
    )
}
