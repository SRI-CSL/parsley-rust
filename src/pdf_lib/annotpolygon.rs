...
use std::rc::Rc;
pub fn annotpolygon_type<'a>(tctx: &'a mut TypeCheckContext) -> Rc<TypeCheck> {
    let assignment_62 = exdataprojection_type(tctx);
    let assignment_61 = exdatamarkupgeo_type(tctx);
    let assignment_60 = exdata3dmarkup_type(tctx);
    let assignment_59 = arrayofarrayspaths_type(tctx);
    let assignment_58 = measuregeo_type(tctx);
    let assignment_57 = measurerl_type(tctx);
    let assignment_56 = TypeCheck::new(tctx, "it", Rc::new(PDFType::PrimType(PDFPrimType::Name)));
    let assignment_55 = bordereffect_type(tctx);
    let assignment_53 = borderstyle_type(tctx);
    let assignment_52 = arrayof_2lineendingsnames_type(tctx);
    let assignment_51 = arrayofnumbersgeneral_type(tctx);
    ...
    let dis_18 = TypeCheck::new(
        tctx,
        "path",
        Rc::new(PDFType::Disjunct(vec![assignment_59])),
    );
    let dis_17 = TypeCheck::new(
        tctx,
        "measure",
        Rc::new(PDFType::Disjunct(vec![assignment_57, assignment_58])),
    );
    let dis_16 = TypeCheck::new(tctx, "be", Rc::new(PDFType::Disjunct(vec![assignment_55])));
    let dis_15 = TypeCheck::new(tctx, "ic", Rc::new(PDFType::Disjunct(vec![assignment_54])));
    let dis_14 = TypeCheck::new(tctx, "bs", Rc::new(PDFType::Disjunct(vec![assignment_53])));
    let dis_13 = TypeCheck::new(tctx, "le", Rc::new(PDFType::Disjunct(vec![assignment_52])));
    let dis_12 = TypeCheck::new(
        tctx,
        "vertices",
        Rc::new(PDFType::Disjunct(vec![assignment_51])),
    );
    ...
    let choices_subtype = ChoicePred(
        ...
        vec![PDFObjT::Name(NameT::new(Vec::from("Polygon")))],
    );
