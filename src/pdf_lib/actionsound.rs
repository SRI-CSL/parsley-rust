...
use std::rc::Rc;
pub fn actionsound_type<'a>(tctx: &'a mut TypeCheckContext) -> Rc<TypeCheck> {
    let assignment_bool_25 =
        TypeCheck::new(tctx, "mix", Rc::new(PDFType::PrimType(PDFPrimType::Bool)));
    let assignment_bool_24 = TypeCheck::new(
        tctx,
        "repeat",
        Rc::new(PDFType::PrimType(PDFPrimType::Bool)),
    );
    let assignment_bool_23 = TypeCheck::new(
        tctx,
        "synchronous",
        Rc::new(PDFType::PrimType(PDFPrimType::Bool)),
    );
    let assignment_22 = TypeCheck::new(
        tctx,
        "volume",
        Rc::new(PDFType::PrimType(PDFPrimType::Real)),
    );
    let assignment_21 = stream_type(tctx);
    ...
    let synchronous_field = DictEntry {
        ...
        chk: assignment_bool_23,

        opt: DictKeySpec::Optional,
    };
    let repeat_field = DictEntry {
        key: Vec::from("Repeat"),
        chk: assignment_bool_24,

        opt: DictKeySpec::Optional,
    };
    let mix_field = DictEntry {
        key: Vec::from("Mix"),
