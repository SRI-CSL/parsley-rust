...
use std::rc::Rc;
pub fn actionrendition_type<'a>(tctx: &'a mut TypeCheckContext) -> Rc<TypeCheck> {
    let assignment_25 = stream_type(tctx);
    let assignment_integer_24 =
        TypeCheck::new(tctx, "op", Rc::new(PDFType::PrimType(PDFPrimType::Integer)));
    let assignment_23 = annotscreen_type(tctx);
    let assignment_22 = renditionmedia_type(tctx);
    let assignment_21 = renditionselector_type(tctx);
    ...
    let dis_3 = TypeCheck::new(tctx, "js", Rc::new(PDFType::Disjunct(vec![assignment_25])));
    let dis_2 = TypeCheck::new(tctx, "an", Rc::new(PDFType::Disjunct(vec![assignment_23])));
    let dis_1 = TypeCheck::new(
        tctx,
        "r",
        Rc::new(PDFType::Disjunct(vec![assignment_21, assignment_22])),
    );
    ...
    let op_field = DictEntry {
        key: Vec::from("OP"),
        chk: assignment_integer_24,

        opt: DictKeySpec::Optional,
    };
    let js_field = DictEntry {
        key: Vec::from("JS"),
        chk: dis_3,
        opt: DictKeySpec::Optional,
    };
    TypeCheck::new(
        ...
        Rc::new(PDFType::Dict(vec![
            type_field, s_field, next_field, r_field, an_field, op_field, js_field,
        ])),
    )
}
