...
pub fn iccprofilestream_type<'a>(tctx: &'a mut TypeCheckContext) -> Rc<TypeCheck> {
    ...
    let assignment_3 = indexedcolorspace_type(tctx);
    let assignment_2 = devicencolorspace_type(tctx);
    ...
    let dis_5 = TypeCheck::new(tctx, "f", Rc::new(PDFType::Disjunct(vec![assignment_14])));
    ...
    let choices_n = ChoicePred(
        String::from("Invalid N"),
        vec![
    ...
    let alternate_field = DictEntry {
        key: Vec::from("Alternate"),
        chk: dis_0,
