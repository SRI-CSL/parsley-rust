...
use std::rc::Rc;
pub fn mediaoffsetframe_type<'a>(tctx: &'a mut TypeCheckContext) -> Rc<TypeCheck> {
    ...
    let choices_s = ChoicePred(
        ...
        vec![PDFObjT::Name(NameT::new(Vec::from("F")))],
    );
    ...
    TypeCheck::new(
        tctx,
        "mediaoffsetframe",
        Rc::new(PDFType::Dict(vec![type_field, s_field, f_field])),
    )
}
