...
use std::rc::Rc;
pub fn actiongoto_type<'a>(tctx: &'a mut TypeCheckContext) -> Rc<TypeCheck> {
    let assignment_28 = dest4struct_type(tctx);
    let assignment_27 = dest1struct_type(tctx);
    let assignment_26 = dest0struct_type(tctx);
    let assignment_25 = destxyzstruct_type(tctx);
    let assignment_24 = dest4_type(tctx);
    let assignment_23 = dest1_type(tctx);
    let assignment_22 = dest0_type(tctx);
    let assignment_21 = destxyz_type(tctx);
    ...
    let choices_s = ChoicePred(
        ...
        vec![PDFObjT::Name(NameT::new(Vec::from("GoTo")))],
    );
    ...
    TypeCheck::new(
        ...
        Rc::new(PDFType::Dict(vec![
            type_field, s_field, next_field, d_field, sd_field,
        ])),
    )
}
