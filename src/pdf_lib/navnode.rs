...
use std::rc::Rc;
pub fn navnode_type<'a>(tctx: &'a mut TypeCheckContext) -> Rc<TypeCheck> {
    let assignment_42 = TypeCheck::new(tctx, "dur", Rc::new(PDFType::PrimType(PDFPrimType::Real)));
    let assignment_41 = navnode_type(tctx);
    let assignment_40 = navnode_type(tctx);
    ...
    let choices_type = ChoicePred(
        ...
        vec![PDFObjT::Name(NameT::new(Vec::from("NavNode")))],
    );
    ...
    TypeCheck::new(
        ...
        Rc::new(PDFType::Dict(vec![
            type_field, na_field, pa_field, next_field, prev_field, dur_field,
        ])),
    )
}
