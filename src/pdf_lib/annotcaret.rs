...
use std::rc::Rc;
pub fn annotcaret_type<'a>(tctx: &'a mut TypeCheckContext) -> Rc<TypeCheck> {
    ...
    let assignment_rectangle_52 = mk_rectangle_typchk(tctx);
    let assignments_disjuncts_3 = Rc::new(PDFType::Disjunct(vec![assignment_rectangle_52]));
    ...
    let dis_12 = TypeCheck::new(tctx, "rd", assignments_disjuncts_3);
    ...
    let choices_sy = ChoicePred(
        String::from("Invalid Sy"),
        vec![
    ...
    let choices_subtype = ChoicePred(
        ...
        vec![PDFObjT::Name(NameT::new(Vec::from("Caret")))],
    );
