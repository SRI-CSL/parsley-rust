...
use std::rc::Rc;
pub fn doctimestamp_type<'a>(tctx: &'a mut TypeCheckContext) -> Rc<TypeCheck> {
    ...
    let assignment_5 = arrayof_3integers_type(tctx);
    let assignment_4 = arrayofsignaturereferences_type(tctx);
    let assignment_3 = arrayofintegersgeneral_type(tctx);
    ...
    let dis_0 = TypeCheck::new(tctx, "cert", Rc::new(PDFType::Disjunct(vec![assignment_2])));
    let choices_subfilter = ChoicePred(
        String::from("Invalid SubFilter"),
        vec![PDFObjT::Name(NameT::new(Vec::from("ETSI.RFC3161")))],
    );
    let choices_type = ChoicePred(
        String::from("Invalid Type"),
        vec![PDFObjT::Name(NameT::new(Vec::from("DocTimeStamp")))],
    );
