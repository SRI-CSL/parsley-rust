...
use crate::pdf_lib::arrayofquadpoints::arrayofquadpoints_type;
...
use std::rc::Rc;
pub fn annotredact_type<'a>(tctx: &'a mut TypeCheckContext) -> Rc<TypeCheck> {
    let assignment_60 = exdataprojection_type(tctx);
    let assignment_59 = exdatamarkupgeo_type(tctx);
    let assignment_58 = exdata3dmarkup_type(tctx);
    let assignment_57 = TypeCheck::new(tctx, "da", Rc::new(PDFType::PrimType(PDFPrimType::String)));
    let assignment_bool_56 = TypeCheck::new(
        tctx,
        "repeat",
        Rc::new(PDFType::PrimType(PDFPrimType::Bool)),
    );
    let assignment_55 = TypeCheck::new(
        tctx,
        "overlaytext",
        Rc::new(PDFType::PrimType(PDFPrimType::String)),
    );
    let assignment_54 = xobjectformtype1_type(tctx);
    let assignment_52 = arrayofquadpoints_type(tctx);
    ...
    let assignment_53 = arrayof_4numberscolorannotation_type(tctx);
    ...
    let dis_14 = TypeCheck::new(tctx, "ro", Rc::new(PDFType::Disjunct(vec![assignment_54])));
    let dis_13 = TypeCheck::new(tctx, "ic", Rc::new(PDFType::Disjunct(vec![assignment_53])));
    ...
    let choices_subtype = ChoicePred(
        ...
        vec![PDFObjT::Name(NameT::new(Vec::from("Redact")))],
    );
    ...
    let quadpoints_field = DictEntry {
        key: Vec::from("QuadPoints"),
        chk: dis_12,
        opt: DictKeySpec::Optional,
    };
    ...
    let ro_field = DictEntry {
        key: Vec::from("RO"),
        chk: dis_14,
        opt: DictKeySpec::Optional,
    };
    let overlaytext_field = DictEntry {
        key: Vec::from("OverlayText"),
        chk: assignment_55,

        opt: DictKeySpec::Optional,
    };
    let repeat_field = DictEntry {
        key: Vec::from("Repeat"),
        chk: assignment_bool_56,
