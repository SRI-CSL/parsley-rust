...
use std::rc::Rc;
pub fn timestampdict_type<'a>(tctx: &'a mut TypeCheckContext) -> Rc<TypeCheck> {
    let assignment_0 = TypeCheck::new(tctx, "url", Rc::new(PDFType::PrimType(PDFPrimType::String)));
    let choices_ff = ChoicePred(
        String::from("Invalid Ff"),
        vec![
    ...
    TypeCheck::new(
        tctx,
        "timestampdict",
        Rc::new(PDFType::Dict(vec![url_field, ff_field])),
    )
}
