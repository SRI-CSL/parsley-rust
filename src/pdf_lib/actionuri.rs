...
use std::rc::Rc;
pub fn actionuri_type<'a>(tctx: &'a mut TypeCheckContext) -> Rc<TypeCheck> {
    let assignment_bool_22 =
        TypeCheck::new(tctx, "ismap", Rc::new(PDFType::PrimType(PDFPrimType::Bool)));
    let assignment_21 =
        TypeCheck::new(tctx, "uri", Rc::new(PDFType::PrimType(PDFPrimType::String)));
    ...
    let choices_s = ChoicePred(
        ...
        vec![PDFObjT::Name(NameT::new(Vec::from("URI")))],
    );
