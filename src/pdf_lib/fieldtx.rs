...
use std::rc::Rc;
pub fn fieldtx_type<'a>(tctx: &'a mut TypeCheckContext) -> Rc<TypeCheck> {
    ...
    let assignment_10 = stream_type(tctx);
    ...
    let dis_5 = TypeCheck::new(tctx, "rv", Rc::new(PDFType::Disjunct(vec![assignment_15])));
    let dis_4 = TypeCheck::new(tctx, "aa", Rc::new(PDFType::Disjunct(vec![assignment_12])));
    let dis_3 = TypeCheck::new(tctx, "dv", Rc::new(PDFType::Disjunct(vec![assignment_11])));
    let dis_2 = TypeCheck::new(tctx, "v", Rc::new(PDFType::Disjunct(vec![assignment_10])));
    ...
    let choices_ft = ChoicePred(
        ...
        vec![PDFObjT::Name(NameT::new(Vec::from("Tx")))],
    );
