...
use std::rc::Rc;
pub fn actionnamed_type<'a>(tctx: &'a mut TypeCheckContext) -> Rc<TypeCheck> {
    ...
    let choices_n = ChoicePred(
        ...
        vec![
            PDFObjT::Name(NameT::new(Vec::from("NextPage"))),
            PDFObjT::Name(NameT::new(Vec::from("PrevPage"))),
            PDFObjT::Name(NameT::new(Vec::from("FirstPage"))),
            PDFObjT::Name(NameT::new(Vec::from("LastPage"))),
        ],
    );
    let choices_s = ChoicePred(
        String::from("Invalid S"),
        vec![PDFObjT::Name(NameT::new(Vec::from("Named")))],
    );
    ...
    TypeCheck::new(
        ...
        Rc::new(PDFType::Dict(vec![
            type_field, s_field, next_field, n_field,
        ])),
    )
}
