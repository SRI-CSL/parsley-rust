...
use std::rc::Rc;
pub fn annotstamp_type<'a>(tctx: &'a mut TypeCheckContext) -> Rc<TypeCheck> {
    let assignment_54 = exdataprojection_type(tctx);
    let assignment_53 = exdatamarkupgeo_type(tctx);
    let assignment_52 = exdata3dmarkup_type(tctx);
    let assignment_51 = TypeCheck::new(tctx, "name", Rc::new(PDFType::PrimType(PDFPrimType::Name)));
    ...
    let choices_it = ChoicePred(
        ...
        vec![
            PDFObjT::Name(NameT::new(Vec::from("StampSnapshot"))),
            PDFObjT::Name(NameT::new(Vec::from("StampImage"))),
            PDFObjT::Name(NameT::new(Vec::from("Stamp"))),
        ],
    );
    ...
    let choices_subtype = ChoicePred(
        ...
        vec![PDFObjT::Name(NameT::new(Vec::from("Stamp")))],
    );
