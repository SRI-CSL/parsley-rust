...
use crate::pdf_lib::calgraydict::calgraydict_type;
...
use std::rc::Rc;
pub fn calgraycolorspace_type<'a>(tctx: &'a mut TypeCheckContext) -> Rc<TypeCheck> {
    let assignment_0 = calgraydict_type(tctx);
    let dis_0 = TypeCheck::new(tctx, "1", Rc::new(PDFType::Disjunct(vec![assignment_0])));
    let choices_zero = ChoicePred(
        String::from("Invalid 0"),
        vec![PDFObjT::Name(NameT::new(Vec::from("CalGray")))],
    );
