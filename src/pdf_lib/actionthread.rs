...
use std::rc::Rc;
pub fn actionthread_type<'a>(tctx: &'a mut TypeCheckContext) -> Rc<TypeCheck> {
    let assignment_23 = bead_type(tctx);
    let assignment_22 = thread_type(tctx);
    ...
    let dis_3 = TypeCheck::new(tctx, "b", Rc::new(PDFType::Disjunct(vec![assignment_23])));
    let dis_2 = TypeCheck::new(tctx, "d", Rc::new(PDFType::Disjunct(vec![assignment_22])));
    ...
    TypeCheck::new(
        ...
        Rc::new(PDFType::Dict(vec![
            type_field, s_field, next_field, f_field, d_field, b_field,
        ])),
    )
}
