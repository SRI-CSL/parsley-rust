...
use std::rc::Rc;
pub fn annotlink_type<'a>(tctx: &'a mut TypeCheckContext) -> Rc<TypeCheck> {
    ...
    let assignment_46 = borderstyle_type(tctx);
    let assignment_45 = arrayofquadpoints_type(tctx);
    let assignment_43 = dest4_type(tctx);
    let assignment_42 = dest1_type(tctx);
    let assignment_41 = dest0_type(tctx);
    let assignment_40 = destxyz_type(tctx);
    ...
    let assignment_44 = actionuri_type(tctx);
    ...
    let dis_12 = TypeCheck::new(tctx, "bs", Rc::new(PDFType::Disjunct(vec![assignment_46])));
    let dis_11 = TypeCheck::new(
        tctx,
        "quadpoints",
        Rc::new(PDFType::Disjunct(vec![assignment_45])),
    );
    let dis_10 = TypeCheck::new(tctx, "pa", Rc::new(PDFType::Disjunct(vec![assignment_44])));
    ...
    let choices_subtype = ChoicePred(
        ...
        vec![PDFObjT::Name(NameT::new(Vec::from("Link")))],
    );
