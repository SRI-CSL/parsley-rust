...
use crate::pdf_lib::movie::movie_type;
use crate::pdf_lib::movieactivation::movieactivation_type;
...
use std::rc::Rc;
pub fn annotmovie_type<'a>(tctx: &'a mut TypeCheckContext) -> Rc<TypeCheck> {
    let assignment_25 = exdataprojection_type(tctx);
    let assignment_24 = exdatamarkupgeo_type(tctx);
    let assignment_23 = exdata3dmarkup_type(tctx);
    let assignment_22 = movieactivation_type(tctx);
    let assignment_21 = movie_type(tctx);
    ...
    let dis_9 = TypeCheck::new(tctx, "a", Rc::new(PDFType::Disjunct(vec![assignment_22])));
