...
use crate::pdf_lib::arrayofocgstate::arrayofocgstate_type;
...
use std::rc::Rc;
pub fn actionsetocgstate_type<'a>(tctx: &'a mut TypeCheckContext) -> Rc<TypeCheck> {
    let assignment_bool_22 = TypeCheck::new(
        tctx,
        "preserverb",
        Rc::new(PDFType::PrimType(PDFPrimType::Bool)),
    );
    let assignment_21 = arrayofocgstate_type(tctx);
    ...
    let choices_s = ChoicePred(
        ...
        vec![PDFObjT::Name(NameT::new(Vec::from("SetOCGState")))],
    );
    ...
    let preserverb_field = DictEntry {
        key: Vec::from("PreserveRB"),
