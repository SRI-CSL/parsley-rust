...
use crate::pdf_lib::soundobject::soundobject_type;
use crate::pdf_lib::stream::stream_type;
use std::rc::Rc;
pub fn annotsound_type<'a>(tctx: &'a mut TypeCheckContext) -> Rc<TypeCheck> {
    let assignment_56 = exdataprojection_type(tctx);
    let assignment_55 = exdatamarkupgeo_type(tctx);
    let assignment_54 = exdata3dmarkup_type(tctx);
    let assignment_53 = TypeCheck::new(tctx, "name", Rc::new(PDFType::PrimType(PDFPrimType::Name)));
    let assignment_52 = soundobject_type(tctx);
