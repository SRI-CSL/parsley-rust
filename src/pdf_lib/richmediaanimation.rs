...
use std::rc::Rc;
pub fn richmediaanimation_type<'a>(tctx: &'a mut TypeCheckContext) -> Rc<TypeCheck> {
    let assignment_1 = TypeCheck::new(tctx, "speed", Rc::new(PDFType::PrimType(PDFPrimType::Real)));
    ...
    let choices_type = ChoicePred(
        ...
        vec![PDFObjT::Name(NameT::new(Vec::from("RichMediaAnimation")))],
    );
    ...
    let playcount_field = DictEntry {
        key: Vec::from("PlayCount"),
        chk: assignment_integer_0,

        opt: DictKeySpec::Optional,
    };
    let speed_field = DictEntry {
        key: Vec::from("Speed"),
        chk: assignment_1,
