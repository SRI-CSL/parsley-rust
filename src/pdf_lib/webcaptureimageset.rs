...
use std::rc::Rc;
pub fn webcaptureimageset_type<'a>(tctx: &'a mut TypeCheckContext) -> Rc<TypeCheck> {
    let assignment_6 = arrayofintegersgeneral_type(tctx);
    ...
    let dis_3 = TypeCheck::new(tctx, "r", Rc::new(PDFType::Disjunct(vec![assignment_6])));
    ...
    let choices_s = ChoicePred(
        ...
        vec![PDFObjT::Name(NameT::new(Vec::from("SIS")))],
    );
    ...
    TypeCheck::new(
        ...
        Rc::new(PDFType::Dict(vec![
            type_field, s_field, id_field, o_field, si_field, ct_field, ts_field, r_field,
        ])),
    )
}
