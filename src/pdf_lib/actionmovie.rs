...
use std::rc::Rc;
pub fn actionmovie_type<'a>(tctx: &'a mut TypeCheckContext) -> Rc<TypeCheck> {
    let assignment_22 = TypeCheck::new(tctx, "t", Rc::new(PDFType::PrimType(PDFPrimType::String)));
    let assignment_21 = annotmovie_type(tctx);
    ...
    let choices_operation = ChoicePred(
        String::from("Invalid Operation"),
        vec![
            PDFObjT::Name(NameT::new(Vec::from("Play"))),
            PDFObjT::Name(NameT::new(Vec::from("Stop"))),
            PDFObjT::Name(NameT::new(Vec::from("Pause"))),
            PDFObjT::Name(NameT::new(Vec::from("Resume"))),
        ],
    );
    let choices_s = ChoicePred(
        String::from("Invalid S"),
        vec![PDFObjT::Name(NameT::new(Vec::from("Movie")))],
    );
    ...
    let annotation_field = DictEntry {
        key: Vec::from("Annotation"),
        chk: dis_1,
        opt: DictKeySpec::Optional,
    };
    ...
    let operation_field = DictEntry {
        key: Vec::from("Operation"),
        chk: TypeCheck::new_refined(
            tctx,
            "operation",
            Rc::new(PDFType::PrimType(PDFPrimType::Name)),
            Rc::new(choices_operation),
        ),
        opt: DictKeySpec::Optional,
    };
    TypeCheck::new(
