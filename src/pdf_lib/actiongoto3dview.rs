...
use std::rc::Rc;
pub fn actiongoto3dview_type<'a>(tctx: &'a mut TypeCheckContext) -> Rc<TypeCheck> {
    let assignment_22 = threedview_type(tctx);
    let assignment_21 = annot3d_type(tctx);
    ...
    let dis_2 = TypeCheck::new(tctx, "v", Rc::new(PDFType::Disjunct(vec![assignment_22])));
    ...
    let choices_s = ChoicePred(
        ...
        vec![PDFObjT::Name(NameT::new(Vec::from("GoTo3DView")))],
    );
    ...
    TypeCheck::new(
        ...
        Rc::new(PDFType::Dict(vec![
            type_field, s_field, next_field, ta_field, v_field,
        ])),
    )
}
