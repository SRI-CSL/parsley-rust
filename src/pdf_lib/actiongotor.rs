...
use std::rc::Rc;
pub fn actiongotor_type<'a>(tctx: &'a mut TypeCheckContext) -> Rc<TypeCheck> {
    let assignment_bool_30 = TypeCheck::new(
        tctx,
        "newwindow",
        Rc::new(PDFType::PrimType(PDFPrimType::Bool)),
    );
    let assignment_29 = dest4struct_type(tctx);
    let assignment_28 = dest1struct_type(tctx);
    let assignment_27 = dest0struct_type(tctx);
    let assignment_26 = destxyzstruct_type(tctx);
    let assignment_25 = dest4_type(tctx);
    let assignment_24 = dest1_type(tctx);
    let assignment_23 = dest0_type(tctx);
    let assignment_22 = destxyz_type(tctx);
    ...
    let choices_s = ChoicePred(
        ...
        vec![PDFObjT::Name(NameT::new(Vec::from("GoToR")))],
    );
