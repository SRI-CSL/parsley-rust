...
use crate::pdf_lib::threedactivation::threedactivation_type;
use crate::pdf_lib::threedreference::threedreference_type;
use crate::pdf_lib::threedstream::threedstream_type;
use crate::pdf_lib::threedunits::threedunits_type;
use crate::pdf_lib::threedview::threedview_type;
use std::rc::Rc;
pub fn annot3d_type<'a>(tctx: &'a mut TypeCheckContext) -> Rc<TypeCheck> {
    let assignment_28 = exdataprojection_type(tctx);
    let assignment_27 = exdatamarkupgeo_type(tctx);
    let assignment_26 = exdata3dmarkup_type(tctx);
    let assignment_25 = measuregeo_type(tctx);
    let assignment_24 = threedunits_type(tctx);
    let assignment_bool_22 =
        TypeCheck::new(tctx, "3di", Rc::new(PDFType::PrimType(PDFPrimType::Bool)));
    let assignment_21 = threedactivation_type(tctx);
    let assignment_20 = threedview_type(tctx);
    let assignment_19 = threedstream_type(tctx);
    let assignment_18 = threedreference_type(tctx);
    ...
    let assignment_rectangle_23 = mk_rectangle_typchk(tctx);
    let assignments_disjuncts_1 = Rc::new(PDFType::Disjunct(vec![assignment_rectangle_23]));
    ...
    let dis_12 = TypeCheck::new(tctx, "geo", Rc::new(PDFType::Disjunct(vec![assignment_25])));
    let dis_11 = TypeCheck::new(tctx, "3du", Rc::new(PDFType::Disjunct(vec![assignment_24])));
    let dis_10 = TypeCheck::new(tctx, "3db", assignments_disjuncts_1);
    let dis_9 = TypeCheck::new(tctx, "3da", Rc::new(PDFType::Disjunct(vec![assignment_21])));
    let dis_8 = TypeCheck::new(tctx, "3dv", Rc::new(PDFType::Disjunct(vec![assignment_20])));
    ...
    let threedi_field = DictEntry {
        key: Vec::from("3DI"),
        chk: assignment_bool_22,

        opt: DictKeySpec::Optional,
    };
    let threedb_field = DictEntry {
        key: Vec::from("3DB"),
        chk: dis_10,
        opt: DictKeySpec::Optional,
    };
    let threedu_field = DictEntry {
        key: Vec::from("3DU"),
        chk: dis_11,
        opt: DictKeySpec::Optional,
    };
    let geo_field = DictEntry {
        key: Vec::from("GEO"),
        chk: dis_12,
        opt: DictKeySpec::Optional,
    };
