...
use crate::pdf_lib::devicendict::devicendict_type;
...
use std::rc::Rc;
pub fn devicencolorspace_type<'a>(tctx: &'a mut TypeCheckContext) -> Rc<TypeCheck> {
    let assignment_9 = devicendict_type(tctx);
    ...
    let dis_3 = TypeCheck::new(tctx, "4", Rc::new(PDFType::Disjunct(vec![assignment_9])));
    ...
    let choices_zero = ChoicePred(
        ...
        vec![PDFObjT::Name(NameT::new(Vec::from("DeviceN")))],
    );
